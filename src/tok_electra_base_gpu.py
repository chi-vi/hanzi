...
class TransformerTaggingTokenizer():
    ...
    def decode_output(self, logits, inputs, model=None):
        output = logits
        if isinstance(output, torch.Tensor):
            output = output.tolist()
        prediction = self.id_to_tags(output, [len(x) + 2 for x in inputs['token']])
        return self.tag_to_span(prediction, inputs)
...
def process_sentence(sentence):
    ...
    preds = toke.decode_output(predictions, inputs)
...
transform = TransformerSequenceTokenizer(tokenizer_path, 'token', ret_subtokens=True, ret_subtokens_group=True, ret_token_span=False, use_fast=True,)
